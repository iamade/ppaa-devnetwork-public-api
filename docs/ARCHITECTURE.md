...
flowchart LR
    subgraph Host["Docker Compose (all ports 127.0.0.1)"]
        ...
        app["app — FastAPI (uvicorn)\n/api/agents · /api/agents/{slug} · /health\n:8005"]
...
2. The static UI (`frontend/`) fetches `/api/agents` and renders the fleet
   client-side. No server-side rendering, no build toolchain.
...
```
src/ppaa_showcase/   FastAPI app (main, config, catalog, health)
